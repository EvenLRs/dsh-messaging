...
纯静态插件架构。设置页随 DSH Web profile 启动自动出现。
...
- `package.json`：新增 `dependencies.ws`（Discord 网关 companion 用）与 `scripts.test`（`npm test` 串联 host-smoke / http-routes / client-smoke）。
