          ...
          React.useEffect(() => {
            ...
            const tick = async () => {
              ...
              try {
                ...
                if (result.status === 'expired' && result.qrcodeUrl) {
                  // 宿主已自动换新码：状态回到 wait 并继续轮询。若停在 'expired'，
                  // effect 的状态守卫（只放行 wait/scaned）会终止轮询——新码上
                  // 再次扫码后的 confirmed 永远收不到，登录卡死在「已过期」。
                  // 无 qrcodeUrl 的终局 expired（会话超时删除）仍走下面的停止分支。
                  setQrUrl(result.qrcodeUrl)
                  setStatus('wait')
