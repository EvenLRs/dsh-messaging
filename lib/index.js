...
export async function apply(ctx) {
    ...
    async function ensureAgent(record) {
      ...
      try {
        ...
        if (!error || error.name !== 'SessionPersistenceNotFoundError') throw error
    ...
    function dispatchLarkEvent(event) {
      if (larkEventType(event) !== 'im.message.receive_v1') return
    ...
    async function startLarkLongConnection(cfg) {
      ...
      const dispatcher = new sdk.EventDispatcher({
        loggerLevel: sdk.LoggerLevel ? sdk.LoggerLevel.info : undefined,
      })
      dispatcher.register({
    ...
    registerUiRoute(['GET', 'POST'], '/__dsh-messaging/config', async (req) => {
      ...
      // 飞书 webhook 凭据由用户填写并原样保存（长连接方式下不参与）。
