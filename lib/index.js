...
function requestOrigin(req) {
  ...
  if (origin) return originOfUrl(origin) || origin
  const referer = headerValue(req && req.headers, 'referer')
  if (referer) return originOfUrl(referer)
  return ''
}
...
function originHostname(origin) {
  try {
    // WHATWG URL.hostname 对 IPv6 会保留方括号（Node: '[::1]'），鉴权比较前先剥掉。
    return unwrapIpv6Hostname(new URL(origin).hostname)
  } catch {
    return ''
  }
}
...
function requestHostOrigin(req) {
  ...
  const proto = req && req.socket && req.socket.encrypted ? 'https' : 'http'
  return originOfUrl(`${proto}://${host}`)
}
...
export async function apply(ctx) {
    ...
    // Origin（或 Referer 推导）必须与当前 Host 同源，且只能是回环地址。
    function authenticateSameOrigin(req, res) {
      ...
      if (!isLoopbackHostname(originHostname(origin))) {
        sendJson(res, 403, { ok: false, error: 'forbidden origin' })
        return false
      }
      return true
    ...
    function registerUiRoute(methods, path, handler) {
      ...
      return registerRoute(path, async (req, res) => {
    ...
    registerUiRoute(['GET', 'POST'], '/__dsh-messaging/config', async (req) => {
      if (req.method === 'GET') return { config: cleanJson(state.config) }
    ...
    registerUiRoute('POST', '/__dsh-messaging/send', async (req) => {
      ...
      return sendOutbound(args.channel, String(args.conversation), String(args.text), args.meta || {})
    })
